...
export interface TradesExternalSearch$Params {
      body?: ApiSearchObjectAltDto | null
}

export function tradesExternalSearch(http: HttpClient, rootUrl: string, params?: TradesExternalSearch$Params, context?: HttpContext): Observable<StrictHttpResponse<ApiMarketJsonResultOfMarketPaginationResultOfListOfMarketSearchResultDto>> {
  const rb = new RequestBuilder(rootUrl, tradesExternalSearch.PATH, 'post');
  if (params) {
