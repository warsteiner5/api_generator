...
export interface TradePlansGetTradesByPlanPostion$Params {
  planPositionId: number;
      body?: ApiSearchObjectAltDto | null
}

export function tradePlansGetTradesByPlanPostion(http: HttpClient, rootUrl: string, params: TradePlansGetTradesByPlanPostion$Params, context?: HttpContext): Observable<StrictHttpResponse<ApiMarketJsonResultOfMarketPaginationResultOfListOfMarketSearchResultDto>> {
  const rb = new RequestBuilder(rootUrl, tradePlansGetTradesByPlanPostion.PATH, 'post');
  if (params) {
    rb.path('planPositionId', params.planPositionId, {});
