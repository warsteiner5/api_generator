...
export interface CompetetiveListGetEquivalentCompetetiveLists$Params {
  requirementRequestId: number;
}

export function competetiveListGetEquivalentCompetetiveLists(http: HttpClient, rootUrl: string, params: CompetetiveListGetEquivalentCompetetiveLists$Params, context?: HttpContext): Observable<StrictHttpResponse<ApiMarketJsonResultOfListOfCompetitiveListItemDto>> {
  const rb = new RequestBuilder(rootUrl, competetiveListGetEquivalentCompetetiveLists.PATH, 'get');
  if (params) {
