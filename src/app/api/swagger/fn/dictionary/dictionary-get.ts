...
export interface DictionaryGet$Params {
  UsedClassificatorType?: ApiUsedClassificatorTypeEnum;
  Value?: string | null;
}

export function dictionaryGet(http: HttpClient, rootUrl: string, params?: DictionaryGet$Params, context?: HttpContext): Observable<StrictHttpResponse<Blob>> {
  const rb = new RequestBuilder(rootUrl, dictionaryGet.PATH, 'get');
  if (params) {
