import { ApproveCustomerDealOnDemandOfProviderRequest } from '../../models/approve-customer-deal-on-demand-of-provider-request.interface';
import { ApiApproveCustomerDealOnDemandOfProviderRequestDto } from '../../../swagger/models/api-approve-customer-deal-on-demand-of-provider-request-dto';
import { apiSignatureForDealDtoAdapter } from './api-signature-for-deal-dto.adapter';

export const apiApproveCustomerDealOnDemandOfProviderRequestDtoAdapter = (source?: ApproveCustomerDealOnDemandOfProviderRequest | null): ApiApproveCustomerDealOnDemandOfProviderRequestDto => {
  return {
    OrganizationId: source?.organizationId,
    SignatureForDealDto: source?.signatureForDealDto === null ? undefined : apiSignatureForDealDtoAdapter(source?.signatureForDealDto),
  };
}
