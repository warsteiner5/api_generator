...
import { apiStandartAltDtoAdapter } from './api-standart.adapter';

export const apiPurchaseObjectCardAltDtoAdapter = (source?: PurchaseObjectCardAlt | null): ApiPurchaseObjectCardAltDto => {
  return {
    ...
    Standarts: source?.standarts?.map((item) => apiStandartAltDtoAdapter(item)),
    Sum: source?.sum,
