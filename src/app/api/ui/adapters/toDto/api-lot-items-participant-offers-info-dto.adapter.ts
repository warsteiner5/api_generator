...
import { adaptApiLinkOfferToLotItemDto } from './api-link-offer-to-lot-item-dto.adapter';

export function adaptApiLotItemsParticipantOffersInfoDto(source?: LotItemsParticipantOffersInfo | null): ApiLotItemsParticipantOffersInfoDto {
  return {
    ...
    ParticipantOffers: (source?.participantOffers ?? []).map((item) => adaptApiLinkOfferToLotItemDto(item)),
  };
}
